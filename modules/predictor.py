import numpy as np, joblib, pandas as pd, os
...
def train():
    ...
    if len(df) < 500: raise ValueError("Need 500+ games")
    ...
    for _, r in df.iterrows():
        ...
        feats = _feats(r, past)
        X.append(feats); yw.append(1 if r['home_score'] > r['away_score'] else 0); yt.append(r['total_points'])
    X = np.array(X); split = int(0.8*len(X))
    from xgboost import XGBClassifier, XGBRegressor
    wm = XGBClassifier(n_estimators=200, max_depth=5, random_state=42); wm.fit(X[:split], np.array(yw)[:split])
    tm = XGBRegressor(n_estimators=200, max_depth=5, random_state=42); tm.fit(X[:split], np.array(yt)[:split])
...
def load():
    return joblib.load(f"{MODEL_DIR}/win_{VER}.pkl"), joblib.load(f"{MODEL_DIR}/total_{VER}.pkl"), joblib.load(f"{MODEL_DIR}/sigma_{VER}.pkl")
