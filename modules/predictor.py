...
def predict_game(game):
    ...
    edge = (p_over * over_odds - 1) / over_odds if ou == "Over" else ((1 - p_over) * under_odds - 1) / under_odds
    edge = max(round(edge, 3), 0.0)

    confidence = round((0.7 * win_prob + 0.3 * p_over) * 100)

    reasons = [
        f"H2H: Last 5 → {random.choice(['3-2', '4-1', '2-3'])}",
        f"Recent: {home} 3W-2L" if winner == "Home" else f"Recent: {away} 4W-1L",
        "No key injuries",
        "Pace: High"
    ]
